...
class SocketIOManager: NSObject {
    ...
    static let manager = SocketManager(socketURL: URL (string: "https://fathomless-ocean-79169.herokuapp.com/")!)
    ...
    func checkUserName(nickname: String) -> OnAckCallback? {
        return socket.emitWithAck("checkUsername", nickname)
    }
    
    func exitUser(clientNickname: String) {
        socket.emit("exitUser",clientNickname)
    }
    ...
    func getChatHistory(last: Int) -> OnAckCallback? {
        return socket.emitWithAck("getChatHistory", last)
